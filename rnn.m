...
for i = 1:length(weights)-1
    weights{i} = 0.01*rand(nbrOfNodesPerLayer(i),nbrOfNodesPerLayer(i+1)) ;
    %weights{i}(:,1) = 0; 
...
hidden_w = 0.01*rand(nbrOfNodesPerLayer(2),nbrOfNodesPerLayer(2)) ;
%hidden_w(:,1) = 0;
%memory length
mem_l = 2;
...
training_mse = zeros(1,nbrOfEpochs_max*64+2);
...
for ep = 1:nbrOfEpochs_max
   for spl = 1:length(Samples)-1
       ...
       predicted(train_idx,:)= NodesActivations{nbrOfLayers};
       ...
       NodesBackPropagatedErrors{nbrOfLayers} = (Samples(:,spl+1)' - NodesActivations{nbrOfLayers}).*gradient;
       ...
       for node=1:length(HiddenBackPropagatedErrors2)
           hidden_w(node,node) = 0;
           HiddenBackPropagatedErrors2(node) = hidden_w(node,:)*(HiddenBackPropagatedErrors*gradient_hidden4(node))';
       end
       ...
       for node=1:length(HiddenBackPropagatedErrors3)
           hidden_w(node,node) = 0;
           HiddenBackPropagatedErrors3(node) = hidden_w(node,:)*(HiddenBackPropagatedErrors2*gradient_hidden3(node))';
       end
       ...
       %Delta_Weights{1}(1,:) = learningRate2*NodesBackPropagatedErrors{2};
       %Delta_Weights{1}(:,1) = Delta_Weights{1}(:,1)./4;
       %Delta_Weights{1}(:,1)
       
       Delta_hidden_w = learningRate2*(HiddenBackPropagatedErrors'*HiddenNodesActivations{4}+ HiddenBackPropagatedErrors2'*HiddenNodesActivations{3}+HiddenBackPropagatedErrors3'*HiddenNodesActivations{2});
